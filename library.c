...
camera_capture_preview (Camera *camera, CameraFile *file, GPContext *context)
{
	...
	case PTP_VENDOR_CANON:
		...
		if (ptp_operation_issupported(params, PTP_OC_CANON_EOS_GetViewFinderData)) {
			int tries = 100, maxtries = 0;
			while (tries--) {
                /**/
				if(100 - tries > maxtries){
					maxtries = 100 - tries;
				}
                ...
                newobject = 0;
                while ((time(NULL)-capture_start)<=60) {
                    int i;
                    ...
                    for (i=sleepcnt;i--;) {
                        ...
                        usleep(10*1000); /* 20 ms */
						printf("\nSleeping, count: %d", sleepcnt);
                    }
                    sleepcnt++; /* incremental back off */
                    if (sleepcnt>3) sleepcnt=3;
                ...
                gp_file_set_name (file, "AUVSIpreview.jpg");
                gp_file_set_mtime (file, time(NULL));
				...
				printf("\nData: %s\nSize: %d\n", (char*)ximage, oi.ObjectCompressedSize);
